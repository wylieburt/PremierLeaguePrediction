...
def create_all_actuals():
    gw_1_actuals = create_gw1_actuals()
    gw_2_actuals = create_gw2_actuals()
    gw_3_actuals = create_gw3_actuals()
    gw_4_actuals = create_gw4_actuals()
    gw_5_actuals = create_gw5_actuals()
    gw_6_actuals = create_gw6_actuals()
    gw_7_actuals = create_gw7_actuals()
    gw_8_actuals = create_gw8_actuals()
    
    all_actuals = [gw_1_actuals, gw_2_actuals, gw_3_actuals, gw_4_actuals, gw_5_actuals, gw_6_actuals, gw_7_actuals, gw_8_actuals]
    
    return all_actuals

def create_gw1_actuals():   
    # Game week 1
...
def create_gw8_actuals():
    # game week 8
    gw_8_actuals_list = [["Sat 18 Oct 04:30","Nott'ham Nottingham Forest", "Chelsea", np.nan, np.nan, "Away Win"],
                      ["Sat 18 Oct 07:00", "Brighton & Hove Albion", "Newcastle United",  np.nan, np.nan, "Away Win"],
                      ["Sat 18 Oct 07:00", "Burnley", "Leeds United", np.nan, np.nan, "Tie"],
                      ["Sat 18 Oct 07:00", "Crystal Palace", "AFC Bournemouth",  np.nan, np.nan, "Tie"],
                      ["Sat 18 Oct 07:00", "Manchester City", "Everton",  np.nan, np.nan, "Home Win"],
                      ["Sat 18 Oct 07:00", "Sunderland", "Wolverhampton Wanderers",  np.nan, np.nan, "Tie"],
                      ["Sat 18 Oct 09:30", "Fulham", "Arsenal",  np.nan, np.nan, "Away Win"], 
                      ["Sun 19 Oct 06:00", "Spurs", "Aston Villa",  np.nan, np.nan, "Home Win"],
                      ["Sun 19 Oct 08:30", "Liverpool", "Manchester United",  np.nan, np.nan, "Home Win"],
                      ["Mon 20 Oct 12:00", "West Ham United", "Brentoford",  np.nan, np.nan, "Tie"]]
